...
def _self_play_episode(config: Config, cur) -> tuple:
    ...
    # ── 1. Obtain ground-truth and embeddings ─────────────────────────────────
    embeddings, ground_truth = _load_sample(config, rng)

    # ── 2. Generate an initial (corrupted) assignment ─────────────────────────
...
def _load_sample(config: Config, rng: np.random.Generator):
    """
    Load (embeddings, ground_truth) for one karyotype sample.

    Strategy:
    1. If a Mask2Former extractor is available and the data directory
       contains images, extract real embeddings.
...
def _load_real_sample(config: Config, rng: np.random.Generator,
    ...
    """
    Load one real karyotype sample using Mask2Former embeddings.
    ...
    return result.embeddings, ground_truth
