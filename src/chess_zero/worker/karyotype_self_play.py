...
def _self_play_episode(config: Config, cur) -> tuple:
    ...
    # ── 1. Obtain ground-truth, embeddings, and Mask2Former predictions ───────
    embeddings, m2f_assignments, ground_truth = _load_sample(config, rng)

    # ── 2. Use Mask2Former's predicted assignments when available; otherwise
    #        introduce controlled random corruptions to simulate classifier
    #        mistakes on synthetic data. ──────────────────────────────────────
...
def _load_sample(config: Config, rng: np.random.Generator):
    """
    Load (embeddings, init_assignments, ground_truth) for one karyotype sample.

    Strategy:
    1. If a Mask2Former extractor is available and the data directory
       contains images, extract real embeddings and use Mask2Former's
       predicted class assignments as the initial state.
    2. Otherwise fall back to synthetic zero embeddings + canonical GT,
       with ``init_assignments`` returned as ``None`` (caller generates
    ...
    # Synthetic fallback — init_assignments will be generated by the caller
    mc = config.model
    embeddings = rng.standard_normal(
        (N_CHROMOSOMES, mc.embedding_dim)).astype(np.float32)
    # Randomly choose XX (female) or XY (male)
    n_sex_x = 2 if rng.random() > 0.5 else 1
    ground_truth = build_ground_truth_assignments(n_sex_chromosomes_x=n_sex_x)
    return embeddings, None, ground_truth


def _load_real_sample(config: Config, rng: np.random.Generator,
                      data_dir: str, annotation_file: str):
    """
    Load one real karyotype sample using Mask2Former embeddings and predictions.

    Requires:
    - ``config.resource.mask2former_config_file`` pointing to the mmdet config
    - ``config.resource.mask2former_checkpoint_file`` pointing to the .pth
    ...
    (np.ndarray, np.ndarray, np.ndarray)
        ...
        mask2former_assignments of shape (N_CHROMOSOMES,) — predicted class
            labels (1–24) from Mask2Former, used as the initial RL state,
    ...
    # Use Mask2Former's predicted assignments directly as the initial RL state
